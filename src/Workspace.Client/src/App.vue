...
import type { CurrentSessionDto as Session, RegionDto as Region, CardDto as Card, AccountDto as Account, WorkspaceSnapshotDto as Snapshot, AuditEventDto as Audit } from './contracts.generated'
...
async function connect() {
  ...
  if (activeConnection.state === HubConnectionState.Disconnected) {
    if (!handlersRegistered) {
      ...
      activeConnection.onreconnected(async () => { await refresh(); online.value = true })
      activeConnection.onclose(() => { online.value = false; window.setTimeout(connect, 3000) })
      handlersRegistered = true
    }
    try { await activeConnection.start(); await refresh(); online.value = true } catch { online.value = false; window.setTimeout(connect, 3000) }
  }
}
...
async function setUsage(card: Card, status: Card['usageStatus']) {
  if (!online.value) return
  busy.value = true
  try { snapshot.value = await request<Snapshot>(`/api/cards/${card.id}/usage`, { method: 'PUT', body: JSON.stringify({ status, assignMeAsPrimaryOperator: status === 'InUse' }) }); refreshTick.value++ }
...
async function create(path: string, body: object) {
  if (!online.value) return
  busy.value = true; message.value = ''
  try { snapshot.value = await request<Snapshot>(path, { method: 'POST', body: JSON.stringify(body) }); refreshTick.value++; message.value = '狀態已保存。' }
...
const reconcile = () => { if (session.value) void refresh().catch(() => { online.value = false }) }

onMounted(async () => {
  window.addEventListener('focus', reconcile)
  poll = setInterval(reconcile, 15000)
...
onUnmounted(() => { clearInterval(poll); window.removeEventListener('focus', reconcile) })
</script>
...
<template>
  <main>
    ...
    <template v-else>
      ...
      <DataWorkspace :snapshot="snapshot" :online="online" :refresh-tick="refreshTick" @refresh="refresh" v-slot="{ fields, openValue, valueText, recordIds, cardLabel, stages, moveStage, stageId }">
      ...
      <section v-for="account in snapshot?.accounts" :key="account.id" class="account">
        ...
        <div class="cards"><article v-for="card in account.cards.filter(c => !recordIds || recordIds.includes(c.id))" :key="card.id" class="card" draggable="true" @dragstart="drag(card.id, $event)">
          ...
          <div class="field-chips"><button v-for="field in fields" :key="field.id" class="field-chip" :style="{ borderLeftColor: field.color }" :aria-label="`${card.displayName} · ${field.name}`" @click="openValue(field, card.id)">{{ field.name }}：{{ valueText(field, card.id) }}</button></div>
          <div class="actions">

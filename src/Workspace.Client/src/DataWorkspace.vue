...
const props = defineProps<{ snapshot?: WorkspaceSnapshotDto; online: boolean; refreshTick: number }>()
...
async function refresh() {
  ...
  try {
    const nextConfig = await request<ConfigurationDto>('/api/configuration')
    if (sequence !== fetchSequence) return
    config.value = nextConfig
    const id = collectionId.value
    const nextData = await request<CollectionSnapshotDto>(`/api/collections/${id}`)
    if (sequence === fetchSequence && id === collectionId.value) data.value = nextData
  } catch (e) { if (sequence === fetchSequence) message.value = (e as Error).message }
}
watch(() => [props.refreshTick, collectionId.value], () => { void refresh() }, { immediate: true })
...
async function saved() { fieldEditor.value = undefined; viewEditor.value = undefined; editing.value = undefined; await refresh(); emit('refresh') }
...
<template>
  <section v-if="config" class="data-workspace">
    ...
    <template v-else>
      ...
      <slot v-if="active === 'coordination' || (view?.display === 'Board' && data?.collection.kind === 'Cards')" :fields="fields" :open-value="openById" :value-text="textById" :record-ids="active === 'coordination' ? undefined : records.map(x => x.id)" :card-label="config.settings.cardLabel" :stages="config.stages" :move-stage="moveById" :stage-id="stageById"></slot>
      <template v-else-if="data">
        ...
        <div v-if="view?.display === 'Board'" class="cards"><article v-for="record in records" :key="record.id" class="card"><h3>{{ record.name }}</h3><button v-for="field in fields" :key="field.id" class="field-chip" @click="openValue(field, record)">{{ field.name }}：{{ text(field, record) }}</button></article></div>
        <div v-else class="table-scroll" :class="{ compact: view?.display === 'Panel' }"><table><thead><tr><th>名稱</th><th v-if="data.collection.kind === 'Cards'">所屬帳號／階段</th><th v-for="field in fields" :key="field.id"><i class="field-dot" :style="{ background: field.color }"></i>{{ field.name }}<small v-if="field.scope === 'Shared'"> · 共用</small></th><th v-if="data.collection.kind === 'Cards'">區域操作</th></tr></thead><tbody><tr v-for="record in records" :key="record.id" :data-record="record.id"><th><button class="plain" :disabled="!online" @click="rename(data.collection.kind === 'Accounts' ? 'accounts' : data.collection.kind === 'Cards' ? 'cards' : 'records', record.id, record.name, record.version)">{{ record.name }}</button></th><td v-if="data.collection.kind === 'Cards'"><span>{{ snapshot?.accounts.find(x => x.id === record.accountId)?.displayName }}</span><select :value="record.stageId ?? ''" :disabled="busy || !online" aria-label="移動階段" @change="move(record, $event)"><option value="" disabled>未指定</option><option v-for="stage in config.stages" :key="stage.id" :value="stage.id">{{ stage.name }}</option></select></td><td v-for="field in fields" :key="field.id"><button class="cell" :aria-label="`${record.name} · ${field.name}`" @click="openValue(field, record)">{{ text(field, record) }}</button></td><td v-if="data.collection.kind === 'Cards'"><button @click="active = 'coordination'">預約／回村</button></td></tr></tbody></table><p v-if="records.length === 0" class="empty">目前沒有符合條件的資料。</p></div>
      </template>
    </template>
    ...
    <ValueEditor v-if="editing && latestValue" :field="editing.field" :initial="editing.initial" :latest-field="latestField" :latest="latestValue" :record="editing.record" :relations="relationChoices" :online="online" @close="editing = undefined" @saved="saved" @refresh="refresh" />
  </section>
</template>

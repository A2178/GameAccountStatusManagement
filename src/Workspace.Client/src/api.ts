...
export class ApiError extends Error {
  constructor(message: string, public current?: ValueDto | null, public status?: number) { super(message) }
}
export async function request<T>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(body.message ?? `要求失敗（${response.status}）`, body.current, response.status)
  }
